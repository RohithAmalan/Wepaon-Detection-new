...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            if self.imgs[i] is None:
                print(f"WARNING: Camera {s} failed to provide first frame. Using dummy black frame.")
        ...
        valid_imgs = [x for x in self.imgs if x is not None]
        if not valid_imgs:
             print("WARNING: No valid frames read from any source. Check camera connection/permissions.")
             # Fallback to dummy shape to prevent immediate crash, though subsequent reads will likely fail
             s = np.array([[self.img_size, self.img_size]])
        else:
             s = np.stack([letterbox(x, self.img_size, stride=self.stride)[0].shape for x in valid_imgs], 0)  # shapes
    ...
    def __next__(self):
        self.count += 1
        img0 = self.imgs.copy()
        # Removed cv2.waitKey(1) from here as it causes issues in threaded environments or headless
        # and should be handled by the main loop consuming the stream.
